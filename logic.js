// variables are storage values
let board;
let score = 0;
let rows = 4;
let columns = 4;

// this variables will be use to monitor is the user is already won
// If one of the these variables value is true then player won
...
function setGame(){
    ...
    for(let r = 0; r < rows; r++){
        for(let c = 0; c < columns; c++){
            ...
            // add unique id to the tiles
            tile.id = r.toString()+"-"+c.toString();

            //Get the number of a tile
            let num = board[r][c]

            // Use the number to update the tile's apperance
            updateTile(tile,num);

            //add the created element to frontend
	...
	for(let i = 0; i < row.length - 1; i++){
        ...
        */
        ...
        if(row[i] == row[i+1]){
            ...
            row[i+1] = 0;    
            score += row[i]; // this codes modifies the score
        } // [2, 2, 2] -> [4, 0, 2]
     }
     row = filterZero(row);
...
function slideLeft(){
    ...
    for(let r=0; r < rows; r++){
        ...
        // all tiles values are save in a container
        let row = board[r]
        ...
        for(let c = 0; c < columns; c++){
            ...
            let num = board[r][c];
            updateTile(tile,num); // update color usinng updatetile function
...
//for merging, slide up and slidedown will be mainly through column values
function slideUp(){
    for(let c = 0; c < columns; c++){
        ...
        // this loop is used to update the id of the tile, and its color
        for(r = 0; r < rows; r++){
            board[r][c] = row[r];
...
function randomizer(){
    for(let r = 0; r < rows; r++){
        for(let c = 0; c < columns; c++){
            ...
            {
                let num = [2,4,8,16,32];
                let pick_num = Math.abs(Math.random(0,100))
                if(pick_num < 35){
                    board[r][c] = num[0];
                }
                else if(pick_num > 35 && pick_num < 50){
                    board[r][c] = num[1];
                }
                else if(pick_num > 50 && pick_num < 75){
                    board[r][c] = num[2];
                }
                else if(pick_num > 75 && pick_num < 90){
                    board[r][c] = num[3];
                }
                else{
                    board[r][c] = num[4];
...
function handleSlide(e){
    ...
    if(hasLost()){
        setTimeout(()=>{
            alert("Game Over!, You've Lost. Game will restart");
            restartGame();
